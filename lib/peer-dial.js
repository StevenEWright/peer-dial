...
var setupServer = function(){
	...
	app.use(pref+"/apps/:appName/additionalData", cors(self.corsOptionsSsdp));
  app.post(pref+"/apps/:appName/additionalData", function(req, rsp) {
    var appName = req.params["appName"];
    var app = self.delegate.getApp.call(req,appName);
    if (app) {
      console.log(req.text);
      app.additionalData = JSON.parse(req.text);
      rsp.sendStatus(200);
    } else {
      rsp.sendStatus(404);
    }
